    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-agri-900 via-agri-800 to-emerald-800 text-white rounded-3xl p-6 md:p-8 shadow-sm relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="mt-2 text-agri-100 text-sm leading-relaxed">
            ...
            {user?.state || 'India'}
            {user?.land_size_acres ? ` • ${user.land_size_acres} Acres registered` : ''}
          </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          subtitle={totalAcres > 0 ? `${totalAcres.toFixed(1)} Total Acres` : 'No land logged yet'}
          icon={Tractor}
          color="agri"
        />
        ...
        <StatCard
          ...
          subtitle={
            weather?.rainfall !== null && weather?.rainfall !== undefined
              ? `${weather.rainfall} mm rain • ${weather.humidity ?? '--'}% hum.`
        ...
        <StatCard
          ...
          value="1 to 30 Days"
          subtitle="HistGradientBoosting ML"
          icon={TrendingUp}
          color="purple"
        />
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          >
            ...
            <span>{scanningAlerts ? 'Checking...' : 'Scan Price & MSP Alerts'}</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <h4 className="font-bold text-sm text-slate-800">Crop Suitability AI</h4>
          ...
          >
            ...
            <h4 className="font-bold text-sm text-slate-800">Live Mandi Prices</h4>
            <p className="text-xs text-slate-500 mt-1">
          ...
          >
            ...
            <h4 className="font-bold text-sm text-slate-800">Price Prediction</h4>
            <p className="text-xs text-slate-500 mt-1">
          ...
          >
            ...
            <h4 className="font-bold text-sm text-slate-800">MSP Benchmark</h4>
            <p className="text-xs text-slate-500 mt-1">
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-xs">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold text-slate-800">Registered Farm Plots</h3>
            <p className="text-xs text-slate-500">
              Manage your agricultural lands and soil test readings
            </p>
          </div>
          <Link
            to="/farms"
        ...
        {loadingFarms ? (
          <div className="py-8 text-center text-xs text-slate-400">Loading farms...</div>
        ) : farms.length === 0 ? (
          <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            <Tractor className="w-8 h-8 text-slate-400 mx-auto mb-2" />
            <p className="text-sm font-semibold text-slate-700">No farms registered yet</p>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              Add your farm GPS coordinates and soil data to enable AI crop recommendations and local weather forecasts.
            </p>
            <button
              onClick={() => setAddFarmOpen(true)}
              ...
              <tbody className="divide-y divide-slate-100">
                {farms.slice(0, 4).map((farm) => (
                  <tr key={farm.farm_id} className="hover:bg-slate-50/60">
                    ...
                    <td className="py-3 text-slate-600">{farm.area_acres} Acres</td>
